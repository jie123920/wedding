...
class Region extends Service
{
    ...
    public static function GetIdByIp($exchange_rate_id,$ip)
    {
        $params = ['clientip' => $ip,'exchange_rate_id'=>$exchange_rate_id];
        $params = self::encode($params);
        $result = myhelper::sendRequest($params, 'GET', true, SHOP_API_URL . 'region/get-id-by-ip');
        return isset($result['data']) ? $result['data'] : '';
    }
