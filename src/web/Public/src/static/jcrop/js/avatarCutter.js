jQuery.UtrialAvatarCutter = function (config) {
    ...
    this.init = function () {
        ...
        //console.log(api)
        //设置选择框默认位置
        var img_top1 = Math.round((430-$('#' + img_id).height())/2);
        var img_left1 = Math.round((430-$('#' + img_id).width())/2);
        $("#"+img_id).css({});
      // console.log();
      $("#avatar_box").css({paddingTop:img_top1,paddingLeft:img_left1});
        api.animateTo([x, y, x + select_width, y + select_height]);
        
        $("#avatar_box .jcrop-keymgr").css("left","-1900px")
    }
