...
class UcenterController extends CommonController {
    ...
    public function actionRegister() {
        ...
        $adv_key = $adv_key?$adv_key.'-MD':'';
        if($returnData['data'] == true || !myhelper::grepcheck($email)) {
            return $this->result(1, [], 'invalid email or email exists');
        }

        if( !$password ){
            return $this->result(1, [], 'password is empty');
        }
        ...
        if( $returnData['code'] ){
            return $this->result(1, $returnData, 'login failed');
        }
        ...
        return $this->result(0, $returnData['data'], 'login ok');
    }
    ...
    public function actionLogin() {
        ...
        if( !myhelper::grepcheck($email)){
            return $this->result(1, [], 'email is invalid');
        }
        if( !$password ){
            return $this->result(1, [], 'password is empty');
        }
        ...
        if( !$returnData ){
            return $this->result(1, [], 'no data');
        }
        if( $returnData['code'] ){
            return $this->result(1, $returnData, 'no data');
        }
        $this->sessions['userinfo'] = $returnData['data'];
    ...
    public function actionGetpassword(){
        ...
        if (myhelper::grepcheck($email)){
            ...
            if ($checkUser['data'] == 1) {
                ...
                if(!$code){
                    return $this->result(2, [], 'User does not exist');
                }

                $callback_url = \yii::$app->params['MY_URL']['M'].'/Forgotpassword?email='.$email.'&code='.$code;
                ...
                if ($send_result) {
                    return $this->result(0, [], 'Password recovery instructions have been sent to your email');
                } else {
    ...
    public function actionRepassword() {
        if (\Yii::$app->request->isPost) {
            ...
            if(!$verify){
                return $this->result(1, [], 'Please request another password recovery email');
            }

            $res = $this->resetpwd($email, $password);
            if ($res === false) {
                return $this->result(1, [], 'Password Invalid Or Email Invalid');
            } else {
                return $this->result(0, [], 'Password successfully reset.');
            }
        }
    }
